...
export default function HotspotStar({ regions, correctId, onComplete }: Props) {
  ...
  const pick = (id: string) => {
    if (picked) return
    if (id === correctId) { setPicked(id); onComplete() }
  ...
  return (
    <div className="bg-white/80 rounded-lg p-2 shadow">
      <div className="grid grid-cols-2 gap-1.5">
        {regions.map((r) => (
          <button key={r.id} onClick={() => pick(r.id)} disabled={!!picked}
            className={`w-11 h-11 rounded-md border-2 text-xl flex items-center justify-center transition ${
              picked === r.id ? 'bg-green-500 border-green-600 text-white'
              : wrong === r.id ? 'bg-red-400 border-red-500 text-white'
