...
export default function DragFillStar({ pieces, slots, onComplete }: Props) {
  ...
  useEffect(() => {
    if (!completedRef.current && placed.size === pieces.length && pieces.length > 0) {
  ...
  const onUp = () => {
    ...
    if (inSlot && !occupied) {
      ...
      setPlaced((prev) => { const n = new Set(prev); n.add(pid); return n })
    } else {
  ...
  return (
    <div ref={boxRef} className="relative w-full h-full" style={{ touchAction: 'none' }} onPointerMove={onMove} onPointerUp={onUp}>
      ...
      {pieces.filter((p) => placed.has(p.id)).map((p) => {
        const s = slotOf(p.id)
        return <div key={p.id} className="absolute flex items-center justify-center text-lg"
