...
export default function DragFillStar({ data, onComplete }: Props) {
  ...
  return (
    <Box ref={boxRef} sx={{ position: 'relative', width: '100%', height: '100%', touchAction: 'none' }}
      onPointerMove={onMove} onPointerUp={onUp}>
      {slots.map(s => (
        <Box key={s.id} className="drag-slot" sx={{
          position: 'absolute', left: `${s.x}%`, top: `${s.y}%`, width: `${s.w}%`, height: `${s.h}%`,
          border: '2px dashed', borderColor: 'rgba(100,116,139,0.6)', borderRadius: 1, bgcolor: 'rgba(241,245,249,0.4)',
        }} />
      ))}
      {pieces.filter(p => placed.has(p.id)).map(p => {
        const s = slotOf(p.id)
        return (
          <Box key={p.id} sx={{ position: 'absolute', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22,
            left: `${s.x + s.w / 2}%`, top: `${s.y + s.h / 2}%`, transform: 'translate(-50%,-50%)' }}>{p.emoji}</Box>
        )
      })}
      {pieces.filter(p => !placed.has(p.id)).map(p => (
        <Box key={p.id} className="drag-piece"
          onPointerDown={e => onDown(e, p.id)}
          sx={{
            position: 'absolute', width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center',
            borderRadius: 1, bgcolor: 'white', boxShadow: 2, fontSize: 22, userSelect: 'none',
